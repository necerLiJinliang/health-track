...
def cancel_appointment(appointment_id: int, reason: str, db: Session = Depends(get_db)):
    result = crud.cancel_appointment(db, appointment_id=appointment_id, reason=reason)
    if not result:
