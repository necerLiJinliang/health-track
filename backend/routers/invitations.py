...
def create_invitation(invitation: schemas.InvitationCreate, sender_id: int, db: Session = Depends(get_db)):
...
def accept_invitation(invitation_id: int, db: Session = Depends(get_db)):
    ...
    if not result:
        raise HTTPException(status_code=400, detail="Invitation not found, already accepted, or expired")
