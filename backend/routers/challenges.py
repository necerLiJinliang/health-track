...
def create_challenge(challenge: schemas.ChallengeCreate, creator_id: int, db: Session = Depends(get_db)):
...
def add_participant_to_challenge(challenge_id: int, user_id: int, db: Session = Depends(get_db)):
    ...
    result = crud.add_participant_to_challenge(db, challenge_id=challenge_id, user_id=user_id)
    if not result:
        raise HTTPException(status_code=400, detail="User is already a participant or challenge not found")
